...
public class RsaTests3 {
	private static String testFilePath = "src/test/java/ITSecurity/BigInteger/test_files/RSA-Tests-3.txt";
	...
	public static Collection<Object[]> convertTestMap(List<ImportedTestCase> tests) {
		...
		for (ImportedTestCase test : tests) {
			Object[] arr = { test.title, Integer.parseInt(test.map.get("s")), test.map.get("p"), test.map.get("P"),
					...
					test.map.get("K"), test.map.get("l"), test.map.get("L"), test.map.get("f"), test.map.get("F") };
			objects.add(arr);
		}
		return objects;
	}

	public RsaTests3(String title, int size, String p, String decP, String q, String decQ, String e, String decE,
			String d, String decD, String g, String decG, String h, String decH, String i, String decI, String j,
			String decJ, String k, String decK, String l, String decL, String f, String decF) {
		this.title = title;
		this.size = size;
		this.hexVals = new String[] { p, q, e, d, g, h, i, j, k, l, f };
		this.decVals = new String[] { decP, decQ, decE, decD, decG, decH, decI, decJ, decK, decL, decF };
	}
	...
	private void testDecryption(KeyPair keys, BigInt plainText, BigInt cipherText) {
		assertNotEquals(plainText, RsaHandler.decrypt(keys.getD(), keys.getN(), cipherText));
	}
	...
	public void testEncrypt() {
		...
		f.fromDecString(this.decVals[10]);
